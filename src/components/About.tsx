      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
          {features.map((feature, index) => (
            <div
              key={index}
              className={`group relative bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl p-5 md:p-8 border border-gray-200/50 dark:border-gray-700/50 transform cursor-pointer will-change-transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'} hover:-translate-y-2 hover:shadow-lg hover:border-[#9cd4e3]/50 transition-all duration-200 ease-out hover:scale-[1.02]`}
            >
              <div className={`inline-flex p-3 md:p-4 bg-gradient-to-r ${feature.gradient} rounded-xl mb-4 md:mb-6 transition-all duration-200 ease-out group-hover:scale-110 group-hover:rotate-3`}>
